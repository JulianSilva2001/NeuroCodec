...
def train(num_gpus, rank, group_name,
    ...
    if rank == 0:
        ...
        json_directory = os.path.join(log_directory, 'json_files')
        if not os.path.isdir(json_directory):
            os.makedirs(json_directory)
            os.chmod(json_directory, 0o775)
            ...
            if n_iter > 0 and n_iter % log["iters_per_ckpt"] == 0 and rank == 0:
                ...
                if rank == 0:
                    ...
                    metrics = {
                        ...
                        "train_reduced_loss": float(reduced_loss),
                        "gradient_norm": float(grad_norm),
                        "learning_rate": float(optimizer.param_groups[0]["lr"]),
                        "info_nce_loss": float(info_nce.detach().item()),
                        "val_loss": float(val_loss),
                        "training_time_seconds": int(time.time() - time0),
                    }
                    metrics_history.append(metrics)
                    json_path = os.path.join(json_directory, 'iter_{}.json'.format(n_iter))
